...
# 3. Launch (single terminal — frontend served by FastAPI)
python3 -m uvicorn proxy:app --host 127.0.0.1 --port 3000
```

Open `http://localhost:3000` → Enter your [OpenRouter API Key](https://openrouter.ai) → Start hacking.
...
| `Address already in use` | Kill existing process: `kill $(lsof -t -i:3000)` |
