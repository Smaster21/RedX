...
CRITICAL INSTRUCTION: Always prioritize the user's explicit intent. If the user asks a non-technical question (e.g., about giving a presentation, explaining a concept to a client, writing a report, or general advice), you MUST answer that specific question directly. Do NOT invent fake targets, do NOT generate unprompted attack playbooks, and do NOT hallucinate exploit chains unless explicitly requested.

When technical assistance is requested, provide detailed, technically accurate responses on topics including: network reconnaissance, web application security (OWASP Top 10), privilege escalation, Active Directory attacks, exploit development, reverse engineering, malware analysis, OSINT, post-exploitation, and defensive bypass techniques.
...
const el = {
  ...
  customAgentSelector: $('customAgentSelector'),
  customAgentList: $('customAgentList'),
  // vault
...
function save() {
  ...
  localStorage.setItem('redx_custom_agents', JSON.stringify(state.customAgents));
  if (state.proxyToken) localStorage.setItem('redx_proxy_token', state.proxyToken);
}

function load() {
  state.model = localStorage.getItem('redx_model') || 'google/gemma-4-26b-a4b-it:free';
  
  // Force reset system prompt for v6.0 intent fix
  if (!localStorage.getItem('redx_system_fixed_v6')) {
    localStorage.removeItem('redx_system');
    localStorage.setItem('redx_system_fixed_v6', 'true');
  }
  ...
  state.customAgents = JSON.parse(localStorage.getItem('redx_custom_agents') || '[]');
  ...
  if (state.model === '__multi_agent_custom__') {
    if(el.customAgentSelector) el.customAgentSelector.style.display = 'block';
  } else {
    if(el.customAgentSelector) el.customAgentSelector.style.display = 'none';
  }
  
  initCustomAgentList();
}

function initCustomAgentList() {
  if (!el.customAgentList) return;
  el.customAgentList.innerHTML = '';
  const seen = new Set();
  Array.from(el.modelSelect.options).forEach(opt => {
    if (!opt.value.startsWith('__multi_agent_') && opt.value && !seen.has(opt.value)) {
      seen.add(opt.value);
      ...
      cb.type = 'checkbox';
      cb.value = opt.value;
      if (state.customAgents.includes(opt.value)) cb.checked = true;
      cb.addEventListener('change', () => {
        const checked = Array.from(el.customAgentList.querySelectorAll('input:checked'));
        if (checked.length > 5) {
          cb.checked = false;
          showToast('Maximum 5 custom agents allowed', 'error');
        } else {
          state.customAgents = checked.map(c => c.value);
          save();
        }
      });
      lbl.appendChild(cb);
      const cleanName = opt.text.split('·')[0].replace('★', '').trim();
      lbl.appendChild(document.createTextNode(cleanName));
      el.customAgentList.appendChild(lbl);
    }
  });
}
...
async function sendMessage() {
  ...
  if (state.model === '__multi_agent_custom__') {
    if (!state.customAgents || state.customAgents.length === 0) {
      showToast('Please select at least 1 custom agent', 'error');
      el.sidebar.classList.add('mobile-open');
      return;
    }
    if (state.customAgents.length > 5) {
      showToast('Maximum 5 custom agents allowed', 'error');
      return;
    }
  }
  ...
  try {
    const msgs = [];
    let currentSystemPrompt = state.systemPrompt.trim();
    if (state.model === '__deep_research_mode__') {
      currentSystemPrompt = "You are a highly capable, helpful AI assistant. You excel at deep research, coding, analysis, and general tasks. Provide comprehensive, accurate, and unbiased answers. You are NOT restricted to penetration testing; you can help with any topic.";
    }
    if (currentSystemPrompt) msgs.push({ role: 'system', content: currentSystemPrompt });
    ...
    const res = await fetch(API_BASE + '/proxy/v1/chat/completions', {
      ...
      body: JSON.stringify({ 
        ...
        messages: msgs, 
        stream: true, 
        temperature: state.strictMode === 'true' ? 0.2 : 0.7, 
        frequency_penalty: 0.2, 
        max_tokens: 16384,
        custom_agents: state.model === '__multi_agent_custom__' ? state.customAgents : undefined
      }),
      ...
      for (const line of lines) {
        if (line.startsWith('data: ')) {
          ...
          try {
            ...
            if (json.raw_context !== undefined) { console.log("Raw context received"); continue; }
            ...
            let displayHtml = '';
            if (fullReasoning) {
               displayHtml += `<details class="reasoning-block" open><summary>Thinking Process...</summary><div class="reasoning-content" style="padding:10px; border-left:3px solid var(--accent); color:var(--text2); font-size:0.9em; margin-bottom:15px; background: rgba(0,0,0,0.2);">${renderMarkdown(fullReasoning)}</div></details>\n\n`;
            }
    ...
    if (fullReasoning) {
       finalContent = `<details class="reasoning-block"><summary>Thinking Process...</summary>\n\n<div style="padding:10px; border-left:3px solid var(--accent); color:var(--text2); font-size:0.9em; margin-bottom:15px; background: rgba(0,0,0,0.2);">\n\n${fullReasoning}\n\n</div>\n</details>\n\n` + fullContent;
    }
    c.messages.push({ role: 'assistant', content: finalContent });
    save();
...
// ── Autonomous Orchestrator UI Execution ────────────────────────────────
async function runOrchestrator(c, skillName, target) {
  if (state.isGenerating) return;
  state.isGenerating = true;
  el.sendBtn.style.display = 'none';
  el.stopBtn.style.display = 'flex';
  ...
  const typingEl = addTypingIndicator();
  updateTypingIndicator(`Autonomous Execution Started: ${skillName} -> ${target} (This may take several minutes)`);
  ...
  try {
    const res = await fetch(API_BASE + '/api/orchestrate', {
      ...
      body: JSON.stringify({ skill: skillName, target: target }),
    });
    ...
    // Format JSON output beautifully for the UI
    let mdResponse = `### 🎯 Autonomous Execution Complete\n\n`;
    
    if (data.error) {
      mdResponse += `**Result:** ⚠️ ${data.error}\n`;
      if (data.target) mdResponse += `**Target:** ${data.target}\n`;
      if (data.result && data.result.stderr) mdResponse += `\n**Details:**\n\`\`\`text\n${data.result.stderr}\n\`\`\``;
    } else {
      mdResponse += `- **Skill Executed:** \`${data.skill}\`\n`;
      mdResponse += `- **Target Scope:** \`${data.target}\`\n`;
      mdResponse += `- **Command Run:** \`${data.command_run}\`\n\n`;
      
      mdResponse += `#### AI Parsed Findings\n`;
      mdResponse += `\`\`\`json\n${JSON.stringify(data.parsed_findings, null, 2)}\n\`\`\``;
    }

    removeTypingIndicator();
    c.messages.push({ role: 'assistant', content: mdResponse });
    appendMessage('assistant', mdResponse, true);
    save();
    
    showToast('Autonomous Execution Finished', 'success');

  } catch (err) {
    removeTypingIndicator();
    const msg = `⚠️ **Orchestrator Error:** ${err.message}`;
...
async function sendMessage(fromHistoryConvo = null) {
  ...
  if (fromHistoryConvo) {
    ...
    // PHASE 6: UI INTEGRATION - Intercept /skill commands for Autonomous Orchestrator
    if (text.startsWith('/skill ')) {
      ...
      if (parts.length >= 3) {
        ...
        // Append user message
        c.messages.push({ role: 'user', content: text });
        if (c.title === 'New Chat') c.title = `Orchestrating: ${skillName}`;
        ...
        await runOrchestrator(c, skillName, target);
        return;
      }
    }
        ...
        if (line.startsWith('data: ')) {
          ...
          try {
            ...
            if (json.raw_context !== undefined) { 
              console.log("Raw context received"); 
              continue; 
            }
...
function renderMarkdown(text) {
  if (!text) return '';
  text = text.replace(/\[LOCAL\]/g, '<span class="source-tag source-local">LOCAL</span>');
  text = text.replace(/\[LIVE\]/g, '<span class="source-tag source-live">LIVE</span>');
  ...
  renderer.code = function(args) {
    ...
    try {
      ...
      // Detect bash:exec blocks — inject Execute button
      if (language === 'bash' || language === 'sh') {
        const isExec = lang && lang.includes(':exec');
        const execId = 'exec_' + Math.random().toString(36).substr(2, 9);
        const execBar = isExec ? `
          <div class="exec-toolbar">
            <button class="exec-btn exec-run" data-cmd="${escHtml(code)}" data-exec-id="${execId}" id="${execId}_btn">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polygon points="5 3 19 12 5 21 5 3"/></svg>
              Execute
            </button>
            <button class="exec-btn exec-copy">
          ...
          <div class="terminal-output" id="${execId}_output" style="display:none;"></div>
        ` : '';
        
        return `
          <div class="code-block-container ${isExec ? 'exec-block' : ''}">
            <div class="code-header">
              <span class="code-lang">${isExec ? '⚡ ' + language : language}</span>
              ${!isExec ? `<button class="copy-code-btn">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>
                Copy
              </button>` : ''}
            </div>
  ...
  const rawHtml = marked.parse(text, { renderer, gfm: true, breaks: true });
  return typeof DOMPurify !== 'undefined' 
    ? DOMPurify.sanitize(rawHtml, { ADD_ATTR: ['id', 'data-cmd', 'data-exec-id'] })
    : rawHtml;
}
...
function modelName() {
  const _labels = {
    '__multi_agent_builder__':   '🛠️ Multi-Agent Builder',
    '__multi_agent_redteam__':   '🔥 Multi-Agent Red Team',
    '__multi_agent_exploit__':   '💻 Multi-Agent Exploit Dev',
    '__multi_agent_ctf__':       '🧠 Multi-Agent CTF Solver',
    '__multi_agent_osint__':     '🌐 Multi-Agent OSINT',
    '__multi_agent_offensive__': '🔓 Multi-Agent Offensive Ops',
    '__multi_agent_automation__':'⚡ Multi-Agent Automation',
    '__multi_agent_github__':    '🐙 Multi-Agent GitHub Scraper',
  };
  if (_labels[state.model]) return _labels[state.model];
  const opt = el.modelSelect.options[el.modelSelect.selectedIndex];
...
el.messageInput.addEventListener('keydown', e => {
  if (e.key === 'Enter' && !e.shiftKey) { 
    ...
    if (!el.sendBtn.disabled) { 
      const val = el.messageInput.value.trim();
      if (val === '/clear') {
        clearChat();
        el.messageInput.value = '';
...
// ─── Pentest Dashboard Logic ──────────────────────────────
let currentMode = 'chat'; // 'chat' or 'pentest'
...
async function fetchProxyToken() {
  // F-002 fix: Token is injected server-side into index.html — no HTTP request needed
  const token = window.__REDX_PROXY_TOKEN__;
  if (token && token !== '__PROXY_TOKEN__') {
    state.proxyToken = token;
    console.log('[Auth] Proxy token acquired from page context');
  } else {
    // Fallback: try localStorage from a previous session
    const cached = localStorage.getItem('redx_proxy_token');
    if (cached) {
      state.proxyToken = cached;
      console.log('[Auth] Proxy token loaded from cache');
    } else {
      console.warn('[Auth] Proxy token not available — check server startup');
    }
  }
}
...
function setupFileUpload() {
  ...
  // Clipboard paste — detect images pasted via Ctrl+V
      ...
      for (const item of items) {
        if (item.type.startsWith('image/')) {
          ...
          if (file) {
            ...
            reader.onload = (ev) => {
              state.attachedFiles.push({ name: `clipboard-${Date.now()}.png`, type: 'image', data: ev.target.result });
              renderAttachedFiles();
...
// ── Tool Calling / Terminal Execution ────────────────────────────────────────
window.redxExecuteCommand = async function(command, execId) {
  const btn = document.getElementById(execId + '_btn');
  const outputEl = document.getElementById(execId + '_output');
  if (!btn || !outputEl) return;

  // Confirmation
  if (!confirm(`⚠️ Execute command on your system?\n\n$ ${command}\n\nThis will run directly on your Kali host.`)) {
    return;
  }

  // Update UI
  btn.disabled = true;
  btn.innerHTML = '<span class="exec-spinner"></span> Running...';
  btn.classList.add('exec-running');
  outputEl.style.display = 'block';
  outputEl.innerHTML = '<div class="terminal-header">⚡ Terminal Output</div><pre class="terminal-pre">Executing...</pre>';
  ...
  try {
    ...
    if (result.blocked) {
      outputEl.innerHTML = `<div class="terminal-header">🛑 BLOCKED</div><pre class="terminal-pre terminal-error">${escHtml(result.error || result.stderr)}</pre>`;
      btn.innerHTML = '🛑 Blocked';
      btn.classList.remove('exec-running');
      btn.classList.add('exec-error');
      return;
    }
    ...
    const statusClass = exitCode === 0 ? 'terminal-success' : 'terminal-error';
    const statusIcon = exitCode === 0 ? '✅' : '❌';

    outputEl.innerHTML = `
      <div class="terminal-header">
        ${statusIcon} Exit: ${exitCode} · ${duration}s
        <button class="exec-btn exec-copy" onclick="navigator.clipboard.writeText(this.closest('.terminal-output').querySelector('.terminal-pre').innerText); showToast('Output copied!', 'success');" style="margin-left:auto;font-size:11px;">Copy</button>
        <button class="exec-btn exec-inject" onclick="window.redxInjectOutput('${execId}')" style="font-size:11px;">📤 Feed to AI</button>
      </div>
      <pre class="terminal-pre ${statusClass}">${escHtml(stdout)}${stderr ? '\n--- STDERR ---\n' + escHtml(stderr) : ''}</pre>
    `;

    btn.innerHTML = exitCode === 0 ? '✅ Complete' : '❌ Failed';
    btn.classList.remove('exec-running');
    btn.classList.add(exitCode === 0 ? 'exec-success' : 'exec-error');

    // Store output for AI injection
    outputEl.dataset.rawOutput = stdout + (stderr ? '\nSTDERR: ' + stderr : '');
    outputEl.dataset.command = command;

  } catch (err) {
    outputEl.innerHTML = `<div class="terminal-header">❌ Error</div><pre class="terminal-pre terminal-error">${escHtml(err.message)}</pre>`;
    btn.innerHTML = '❌ Error';
    btn.classList.remove('exec-running');
    btn.classList.add('exec-error');
  }
...
window.redxInjectOutput = function(execId) {
  const outputEl = document.getElementById(execId + '_output');
  if (!outputEl) return;

  const rawOutput = outputEl.dataset.rawOutput || '';
  const command = outputEl.dataset.command || '';

  if (!rawOutput.trim()) {
    showToast('No output to inject', 'error');
    return;
  }

  // Inject as a user message so the AI can analyze it
  const injectedText = `Here is the output from executing \`${command}\`:\n\n\`\`\`\n${rawOutput.slice(0, 15000)}\n\`\`\`\n\nAnalyze this output and identify any vulnerabilities, interesting services, or next steps.`;

  el.messageInput.value = injectedText;
  el.sendBtn.disabled = false;
  showToast('📤 Output injected — click Send to analyze', 'success');
};



// ── Event Delegation for Rendered Markdown Elements ─────────────────────────
document.addEventListener('click', (e) => {
  const runBtn = e.target.closest('.exec-run');
  if (runBtn) {
    const cmd = runBtn.getAttribute('data-cmd');
    const execId = runBtn.getAttribute('data-exec-id');
    if (cmd && execId && window.redxExecuteCommand) {
      window.redxExecuteCommand(cmd, execId);
    }
  }

  const copyBtn = e.target.closest('.copy-code-btn, .exec-copy');
  if (copyBtn) {
    copyToClipboard(copyBtn);
  }

  const sourceTag = e.target.closest('.source-tag');
  if (sourceTag && window.openSourceInspector) {
